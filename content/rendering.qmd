...
RStudio will recognize that this is a Quarto project by the precense of the `_quarto.yml` file and will see the "Build" tab.  Click the "Render website" button to render to the `docs` folder.

**Previewing:** You can either click `index.html` in the `docs` folder and specify "preview in browser" or set up RStudio to preview to the viewer panel. To do the latter, go to Tools > Global Options > R Markdown. Then select "Show output preview in: Viewer panel". 